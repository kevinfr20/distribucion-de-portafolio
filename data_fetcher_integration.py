...
class DataFetcher:
    ...
    def __init__(
        ...
        self.factory = DataProviderFactory(
            bvl_api_key=bvl_api_key,
            trading_economics_token=trading_economics_token
        )
    ...
    def fetch_data(
        ...
        **kwargs : dict
        ...
        fetcher = DataFetcher()
        return fetcher.fetch_data_dynamic(tickers, start_date, end_date, include_dividends, **kwargs)
    ...
    def fetch_data_dynamic(
        ...
        **kwargs : dict
        ...
        for market, market_tickers in tickers_by_market.items():
            self.logger.info(f"   Procesando {len(market_tickers)} ticker(s) de {market}...")
            ...
            for ticker in market_tickers:
                ...
                if not data.empty:
                    ...
                    self.logger.warning(f"   ⚠️ No se pudieron descargar datos para {ticker}")
        
        # Consolidar datos
        if all_data:
            consolidated = pd.DataFrame(all_data)
            self.logger.info(f"✅ {len(consolidated)} registros descargados para {len(all_data)} activo(s)")
            return consolidated
        else:
            self.logger.error(f"❌ No se pudieron descargar datos para ninguno de los tickers")
            return pd.DataFrame()
    ...
    def fetch_data_with_dividends(
        ...
        **kwargs : dict
        ...
        self.logger.warning("⚠️ dividends solo disponibles para Yahoo Finance. Otros mercados retornarán DataFrame vacío.")
...
def create_portfolio_from_mixed_tickers(
    ...
    try:
        ...
        raise ImportError("unified_portfolio_colab.py debe estar en el mismo directorio")
    ...
    if not prices.empty:
        try:
            ...
            try:
                ...
                print(f"   Retorno esperado (anualizado): {summary['expected_return']*100:.2f}%")
                ...
                optimal = portfolio.optimize_portfolio('sharpe', update_self_weights=True)
                ...
                print("   (Asegúrate de que unified_portfolio_colab.py está disponible)")
