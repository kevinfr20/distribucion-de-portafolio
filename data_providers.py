    ...
    def get_historical_data(
        ...
        **kwargs : dict
        ...
        try:
            ...
            self.logger.info(
                f"📥 Descargando {ticker} desde Yahoo Finance ({start_date} a {end_date})"
            )
            ...
            if data.empty:
                self.logger.warning(
                    f"⚠️ No hay datos disponibles para {ticker} en el rango especificado"
                )
                return pd.DataFrame()
            ...
            self.logger.error(
                f"❌ Error descargando {ticker} desde Yahoo Finance: {str(e)}"
            )
            return pd.DataFrame()
...
class BVLDataProvider(BaseDataProvider):
    ...
    def _make_request(
        ...
        try:
            ...
            self.logger.error(
                f"❌ Error HTTP en BVL: {response.status_code} - {str(e)}"
            )
            return None
        except Exception as e:
    ...
    def get_historical_data(
        ...
        **kwargs : dict
        ...
        try:
            ...
            self.logger.info(
                f"📥 Descargando {ticker} desde BVL API ({start_date} a {end_date})"
            )
            ...
            self.logger.info(
                f"✅ {len(df)} registros descargados para {ticker} desde BVL"
            )
            return df
    ...
    def get_historical_data(
        ...
        **kwargs : dict
        ...
        try:
            ...
            self.logger.info(
                f"📥 Descargando {ticker} desde Trading Economics ({start_date} a {end_date})"
            )
            ...
            data_list = self._make_request(
                endpoint, params={"d1date": start_date, "d2date": end_date}
            )

            if not data_list:
                self.logger.warning(
                    f"⚠️ No hay datos disponibles para {ticker} en Trading Economics"
                )
                return pd.DataFrame()
            ...
            column_mapping = {"Date": "Date", "Value": "Close", "LastUpdate": "Date"}
            ...
            self.logger.info(
                f"✅ {len(df)} registros descargados para {ticker} desde Trading Economics"
            )
            return df

        except Exception as e:
            self.logger.error(
                f"❌ Error descargando {ticker} desde Trading Economics: {str(e)}"
            )
            return pd.DataFrame()
...
class DataProviderFactory:
    ...
    def __init__(
        ...
        self.trading_economics_provider = TradingEconomicsProvider(
            api_token=trading_economics_token
        )
    ...
    def identify_market(self, ticker: str) -> str:
        ...
        if any(
            keyword in ticker_upper
            for keyword in ["PERU", "BVL", "VOLC", "ALICORP", "FERREYC"]
        ):
        ...
        self.logger.debug(
            f"Mercado identificado para {ticker}: Yahoo Finance (default)"
        )
        return "yahoo"
    ...
    def fetch_data(
        ...
        **kwargs : dict
        ...
        if all_data:
            ...
            self.logger.warning(
                "⚠️ No se pudieron obtener datos para ninguno de los tickers"
            )
            return pd.DataFrame()
...
def create_data_provider_factory(
    bvl_api_key: Optional[str] = None, trading_economics_token: Optional[str] = None
) -> DataProviderFactory:
    """
